...
struct TokenBox
{
  ...
  {
    return access_token.empty() || std::chrono::steady_clock::now() >= expire_at;
  }
...
GmailClient::GmailClient()
{
  ...
  {
    throw std::runtime_error("FATAL: Could not parse client_secret.json: " + std::string(e.what()));
  }
  ...
  if (ft)
  {
    ...
    {
      std::cerr << "⚠️ Could not parse token.json. A new login may be required.\n";
    }
  }
}
...
bool GmailClient::ensureValidToken()
{
  ...
  {
    std::cerr << "❌ No refresh token available. Please run interactive login.\n";
    return false;
  }
...
bool GmailClient::RunInteractiveLogin()
{
  ...
  const std::string scope = "https://www.googleapis.com/auth/gmail.readonly https://www.googleapis.com/auth/gmail.send";
  std::string auth_url = "https://accounts.google.com/o/oauth2/v2/auth?response_type=code" +
  ...
  if (open_browser(auth_url))
    std::cout << "🌐 Browser opened. Please sign in and copy the authorization code.\n";
  else
  ...
  std::string body = "code=" + urlencode(code) + "&client_id=" + urlencode(client_id_) +
                     "&client_secret=" + urlencode(client_secret_) + "&redirect_uri=" + urlencode(redirect) +
  ...
  try
  {
    ...
    token_box_->expire_at = std::chrono::steady_clock::now() +
...
bool GmailClient::SendEmail(const std::string &to, const std::string &subject, const std::string &body)
{
  if (!ensureValidToken())
  {
    return false;
