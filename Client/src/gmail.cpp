...
std::string GmailClient::UploadToDriveAndGetShareableLink(const std::string &file_path)
{
  ...
  // --- Part A: Upload the file ---
  {
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,webViewLink");
    curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
    ...
    try
    {
      ...
      if (j.contains("error"))
      {
        ...
        std::cerr << "Google Drive API returned an error: " << error_message << std::endl;
        curl_easy_cleanup(curl);
        return "";
      }
      ...
      {
        std::cerr << "Error: Drive API response is missing 'id' or 'webViewLink'." << std::endl;
        std::cerr << "Full Response: " << j.dump(2) << std::endl; // Dump for debugging
        curl_easy_cleanup(curl);
        return "";
      }
    ...
    catch (const json::exception &e)
    {
      ...
      std::cerr << "Raw Response: " << upload_response << std::endl; // Log the raw string
      curl_easy_cleanup(curl);
      return "";
    }
  }
